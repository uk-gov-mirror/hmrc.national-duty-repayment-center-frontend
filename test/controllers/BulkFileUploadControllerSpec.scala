...
import models.{CustomsRegulationType, FileTransmissionFailed, FileUpload, FileUploads, NormalMode, NumberOfEntriesType, S3UploadError, UpscanNotification, UserAnswers}
import org.mockito.Matchers.any
import org.mockito.Mockito.when
import org.scalatestplus.mockito.MockitoSugar
import pages.{ClaimantTypePage, CustomsRegulationTypePage, NumberOfEntriesTypePage}
...
import services.{FileUploaded, UploadFile, WaitingForFileVerification}
...
class BulkFileUploadControllerSpec extends SpecBase with MockitoSugar {
  ...
  "GET /file-rejected/:id" should {

    "set current file upload status as rejected and return 204 NoContent" in {
      lazy val rejectFileUrl = s"${routes.FileUploadController.asyncMarkFileUploadAsRejected(userAnswersId).url}?key=11370e18-6e24-453e-b45a-76d3e32ea33d&errorCode=ABC123&errorMessage=ABC+123"
      val fileUploadedState = UploadFile(
      ...
      val userAnswers = UserAnswers(userAnswersId).set(CustomsRegulationTypePage, CustomsRegulationType.UKCustomsCodeRegulation).success.value
        .copy(fileUploadState = Some(fileUploadedState))
      ...
      running(application) {
        ...
        val request = buildRequest(GET, rejectFileUrl)
        val result = route(application, request).value
        ...
        answers.fileUploadState.get mustBe expectedState
      }
      application.stop()
    }
  }

  "GET /file-verification/:id" in {
    lazy val fileVerificationUrl = s"${routes.FileUploadController.asyncWaitingForFileVerification(userAnswersId).url}"
    val fileUploadedState = UploadFile(
      "11370e18-6e24-453e-b45a-76d3e32ea33d",
      UploadRequest(href = "https://s3.bucket", fields = Map("callbackUrl" -> "https://foo.bar/callback")),
      FileUploads(files =
    ...
    val userAnswers = UserAnswers(userAnswersId).set(CustomsRegulationTypePage, CustomsRegulationType.UKCustomsCodeRegulation).success.value
      .copy(fileUploadState = Some(fileUploadedState))
    val expectedState = WaitingForFileVerification(
      "11370e18-6e24-453e-b45a-76d3e32ea33d",
      UploadRequest(href = "https://s3.bucket", fields = Map("callbackUrl" -> "https://foo.bar/callback")),
    ...
    running(application) {
      val sessionRepo = application.injector.instanceOf[SessionRepository]
      sessionRepo.set(userAnswers)
      val request = buildRequest(GET, fileVerificationUrl)
      val result = route(application, request).value
      status(result) mustEqual 202
      contentAsString(result).isEmpty mustBe true
      val answers = Await.result(sessionRepo.get(userAnswersId), 10 seconds).value
      answers.fileUploadState.get mustBe expectedState
    }
  ...
  "GET /file-verification/:reference/status" should {
    "return file verification status" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(CustomsRegulationTypePage, CustomsRegulationType.UKCustomsCodeRegulation).success.value
        .copy(fileUploadState = Some(fileUploadState))
