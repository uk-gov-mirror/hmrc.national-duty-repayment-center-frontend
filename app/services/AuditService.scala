...
object NDRCFrontendEvent extends Enumeration {
  val NDRCFrontendSomethingHappened = Value
  type NDRCFrontendEvent = Value
}

@Singleton
class AuditService @Inject() (val auditConnector: AuditConnector) {

  import NDRCFrontendEvent._

  private[services] def auditEvent(
    event: NDRCFrontendEvent,
  ...
  private[services] def createEvent(
    ...
    DataEvent(auditSource = "ndrc-frontend", auditType = event.toString, tags = tags, detail = detail)
  }
